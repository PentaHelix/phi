...
class PlayerInventory extends Object {
  ...
  private var itemBitmaps: Array<Bitmap> = [];
  private var itemTiles: Array<Tile>;
  ...
  public function new () {
    ...
    var items = Res.load("items.png").toTile();
    itemTiles = [
			 for(x in 0 ... C.ITEM_COUNT)
			 for(y in 0 ... 1)
			 items.sub(x * 16, y * 16, 16, 16).center()
		];
