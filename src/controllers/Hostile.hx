...
class Hostile implements Controller {
  ...
  public function getAction (): Action {
    if (target == null) target = Manager.actors.getNearestNonHostile(self.transform.pos);
    if (target == null) return new WaitAction();

    var path = Manager.map.findPath(self.transform.pos, target.transform.pos);
    if (path == null || path.length == 0) return new WaitAction();
    
    if (Manager.map.at(path[1]).actor == target) return new AttackAction(self, target);
    if (Manager.map.at(path[1]).actor != null) return new WaitAction();
